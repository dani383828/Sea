...
async def clear_data(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id == ADMIN_ID:
        ...
        await update.message.reply_text("⛔ فقط ادمین می‌تونه داده‌ها رو پاک کنه!")
...
async def top_captains(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, (player_id, data) in enumerate(sorted_players, 1):
        ...
        if player_id != user_id:
            text += f"⚔️ برای دعوت به جنگ دوستانه روی '{username}' کلیک کن!\n"
    
    await update.message.reply_text(text)
    save_data(context)

# 📌 هندلر برای درخواست جنگ دوستانه از لیست
async def handle_friend_request(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.message.from_user.id
    text = update.message.text.strip()
    user_data = context.bot_data.get("user_data", {})
    target_id = next((pid for pid, data in user_data.items() if data.get("username") == text), None)
    ...
    if target_id and target_id != user_id:
        ...
        text = (
            f"🏴‍☠️ کاربر {requester_name} با این اطلاعات کشتی بهت درخواست جنگ دوستانه داده! قبول می‌کنی؟\n"
        ...
        await update.message.reply_text(f"⚔️ درخواست جنگ دوستانه برای {text} ارسال شد!")
    else:
        await update.message.reply_text("⛔ ناخدا پیدا نشد یا نمی‌تونی به خودت درخواست بدی!")
    save_data(context)
...
async def strategy_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ["🗡️ حمله گرایانه", "🛡️ دفاعی"],
        ["🔙 بازگشت به منو"]
    ]
    reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True, one_time_keyboard=False)
    
    strategy_text = {
        "aggressive": "🗡️ حمله گرایانه",
        "defensive": "🛡️ دفاعی",
        "balanced": "⚖️ متوازن"
    }
    ...
    text = (
        f"⚔️ استراتژی فعلی: {strategy_text.get(current_strategy, '⚖️ متوازن')}\n"
...
async def send_game_reports(update: Update, context: ContextTypes.DEFAULT_TYPE, opponent_name: str, cannons: int, energy: int, opponent_cannons: int):
    ...
    battle_reports = [
        "🏴‍☠️ ناخدا، کشتی دشمن از مه بیرون اومد! آماده نبرد شو!",
        "⚔️ نیروهای دشمن با طناب به کشتی‌مون چنگ زدن! شمشیرها رو آماده کن!",
        "💥 با یه شلیک توپ، عرشه دشمن شعله‌ور شد!",
        "⛵️ دشمن داره از پهلو نزدیک می‌شه! دفاع رو تقویت کن!",
        ...
        "💥 کشتی دشمن داره غرق می‌شه، یه شلیک دیگه بزن!",
        "🏹 نیروهای دشمن تو عرشه‌مون نفوذ کردن، به جنگشون برو!",
        ...
        "🌳 با شلیک دقیق، دکل دشمن شکسته شد!",
        "🏴‍☠️ نیروهای دشمن دارن تسلیم می‌شن، جلو برو!",
        ...
        "🌊 یه موج، قایق دشمن رو واژگون کرد!",
        "🎯 دشمن داره با نیزه حمله می‌کنه، دفاع کن!",
        "💥 با شلیک توپ، ۳ نفر از اونا تو دریا غرق شدن!",
        "🏚️ ناخدا، یه گروه کوچک از دشمن تو انبار پنهان شدن!",
        "💣 دشمن با باروت حمله می‌کنه، عقب‌نشینی کن!",
        "🪓 با تبر هامون، ۷ نفر از اونا رو از بین بردیم!",
        "🔥 کشتی دشمن داره آتش می‌گیره، ادامه بده!",
        ...
        "🎯 دشمن با نیزه به عرشه حمله می‌کنه!",
        "💥 با شلیک، ۴ نفر از اونا کشته شدن!",
        "🌊 ناخدا، یه موج بزرگ دشمن رو غرق کرد!",
        "🌫️ دشمن با دود ما رو گیج می‌کنه!",
...
async def handle_game_options(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif choice == "⚔️ استراتژی":
        await strategy_menu(update, context)
    
    elif choice in ["🗡️ حمله گرایانه", "🛡️ دفاعی"]:
...
async def handle_friend_game(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith("reject_friend_game_"):
        ...
        await context.bot.send_message(requester_id, f"🏴‍☠️ کاربر {context.bot_data['usernames'].get(user_id, 'ناشناس')} درخواست جنگ دوستانه‌ات رو رد کرد.")
...
async def shop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"🥈 شمش نقره: {silver}\n\n"
        "🌟 گزینه‌ها:\n"
        "1. 💎 خرید جم (۲۵ جم = ۵ ترون، ۵۰ جم = ۸ ترون، ۱۰۰ جم = ۱۴ ترون)\n"
        "2. ☄️ خرید توپ:\n"
        "   - ۱ توپ = ۳ 💎 جم\n"
        "   - ۳ توپ = ۷ 💎 جم\n"
        "   - ۱۰ توپ = ۱۸ 💎 جم\n"
...
# 📌 هندلر برای پردازش خرید از فروشگاه
async def handle_shop_purchase(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif choice.startswith("💎 خرید جم"):
        gems = 0
        tron = 0
        if "۲۵ جم" in choice:
            gems, tron = 25, 5
        elif "۵۰ جم" in choice:
            gems, tron = 50, 8
        elif "۱۰۰ جم" in choice:
            gems, tron = 100, 14
        if gems:
            user_data["pending_gems"] = gems
            await update.message.reply_text(f"💎 لطفاً {tron} ترون به آدرس زیر ارسال کنید و فیش پرداخت رو بفرستید:\nTJ4xrw8KJz7jk6FjkVqRw8h3Az5Ur4kLkb")
    
    elif choice.startswith("☄️ خرید توپ"):
        if "۱ توپ" in choice and user_data["gems"] >= 3:
            user_data["gems"] -= 3
            user_data["cannons"] += 1
            await update.message.reply_text("☄️ 💎 یه توپ با ۳ جم خریدی!")
        elif "۳ توپ" in choice and user_data["gems"] >= 7:
            user_data["gems"] -= 7
            user_data["cannons"] += 3
            await update.message.reply_text("☄️ 💎 ۳ توپ با ۷ جم خریدی!")
        elif "۱۰ توپ" in choice and user_data["gems"] >= 18:
            user_data["gems"] -= 18
            user_data["cannons"] += 10
            await update.message.reply_text("☄️ 💎 ۱۰ توپ با ۱۸ جم خریدی!")
        elif "۲۰ توپ" in choice and user_data["gems"] >= 30:
            user_data["gems"] -= 30
            user_data["cannons"] += 20
            await update.message.reply_text("☄️ 💎 ۲۰ توپ با ۳۰ جم خریدی!")
        else:
...
async def ship_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"🏆 میانگین پیروزی: {win_rate:.1f}%\n"
        f"⚡ انرژی: {energy}%\n"
        f"⚔️ استراتژی فعلی: {strategy_text.get(strategy, '⚖️ متوازن')}\n"
        f"🗡️ قدرت حمله: {attack}%\n"
        f"🛡️ قدرت دفاع: {defense}%"
    )
    await update.message.reply_text(text)
...
async def warriors_energy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        "⚡ انرژی جنگجویان: {energy}%\n"
        "🌟 اگر جنگجویان شما خسته‌اند، می‌تونی خوراکی بخری:\n"
        "1. ۱ بسته بیسکویت دریایی (۲۵٪ ⚡ انرژی) - قیمت: ۰ 🪙 طلا، ۴ 🥈 نقره\n"
        "2. ۵ عدد ماهی خشک (۳۵٪ ⚡ انرژی) - قیمت: ۱ 🪙 طلا، ۱ 🥈 نقره\n"
        "3. ۳ بسته میوه خشک‌شده (۳۰٪ ⚡ انرژی) - قیمت: ۱ 🪙 طلا، ۰ 🥈 نقره\n"
        "4. ۱۰ قالب پنیر کهنه (۵۰٪ ⚡ انرژی) - قیمت: ۱ 🪙 طلا، ۳ 🥈 نقره\n"
        "5. ۱۰ بطری آب (۲۰٪ ⚡ انرژی) - قیمت: ۰ 🪙 طلا، ۳ 🥈 نقره\n"
    )
    if energy >= 100:
        text += "\n✅ جنگجویان شما پر از انرژی‌اند!"
...
async def handle_food_purchase(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    now = datetime.now()
    items = {
        "۱ بسته بیسکویت دریایی": (0, 4, 25, "biscuit"),
        "۵ عدد ماهی خشک": (1, 1, 35, "fish"),
        "۳ بسته میوه خشک‌شده": (1, 0, 30, "fruit"),
        "۱۰ قالب پنیر کهنه": (1, 3, 50, "cheese"),
        "۱۰ بطری آب": (0, 3, 20, "water")
    }
    
    if choice in items:
        gold_cost, silver_cost, energy_gain, item_id = items[choice]
        gold = user_data["gold"]
        silver = user_data["silver"]
        energy = user_data["energy"]
        last_time = user_data.get("last_purchase", {}).get(item_id)
        
        if (not last_time or (now - last_time).total_seconds() >= 24 * 3600) and gold >= gold_cost and silver >= silver_cost:
            user_data["gold"] -= gold_cost
            user_data["silver"] -= silver_cost
            user_data["energy"] = min(100, energy + energy_gain)
            user_data["last_purchase"][item_id] = now
            await update.message.reply_text(f"✅ 🌟 خرید انجام شد! {energy_gain}% ⚡ انرژی اضافه شد.")
        else:
            await update.message.reply_text("⛔ 🪙 🥈 منابع کافی نیست یا ۲۴ ساعت نگذشته!")
...
application.add_handler(MessageHandler(filters.Regex("^(⛵️ دریانوردی|☄️ توپ|🔙 بازگشت به منو|⚔️ استراتژی)$"), handle_game_options))
application.add_handler(MessageHandler(filters.Regex("^(🗡️ حمله گرایانه|🛡️ دفاعی)$"), set_strategy))
application.add_handler(MessageHandler(filters.Regex("^(0%|10%|20%|35%|50%|65%|80%|90%|100%)$"), handle_strategy_input))
application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND & ~filters.Regex("^(🛒|📕|⚡️|⚔️|🏴‍☠️|⛵️ دریانوردی|☄️ توپ|🔙 بازگشت به منو|⚔️ استراتژی|🗡️ حمله گرایانه|🛡️ دفاعی|0%|10%|20%|35%|50%|65%|80%|90%|100%|\n1\.|2\.|3\.|4\.|5\.)$") & filters.UpdateType.MESSAGE, handle_username))
application.add_handler(MessageHandler(filters.Regex("^(1\.|2\.|3\.|4\.|5\.)"), handle_food_purchase))
application.add_handler(MessageHandler(filters.Regex("^(💎 خرید جم|☄️ خرید توپ|🔙 بازگشت به منو)$"), handle_shop_purchase))
application.add_handler(MessageHandler(filters.TEXT & filters.UpdateType.MESSAGE, handle_friend_request))
