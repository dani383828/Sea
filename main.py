...
async def handle_strategy_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "waiting_for_attack_strategy":
        ...
        user_data["defense_strategy"] = 100 - value  # مجموع حمله و دفاع 100 باشد
        ...
        user_data["attack_strategy"] = 100 - value  # مجموع حمله و دفاع 100 باشد
...
async def search_opponent(update: Update, context: ContextTypes.DEFAULT_TYPE, cannons: int, energy: int, drones: int):
    ...
    opponent_id = None
    opponent_name = (
        "دزد دریایی ناشناس" if language == "persian" else
        "Unknown Pirate"
    )
    ...
    opponent_defense = 100 - opponent_attack  # مجموع حمله و دفاع 100 باشد
...
async def warriors_energy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        "هر ۳۰ دقیقه، ۵٪ انرژی اضافه می‌شه! ⏳\n"
        "برای بازیابی سریع‌تر، به فروشگاه برو و جم بخر! 🛒"
    ) if language == "persian" else (
        f"⚡ Warriors' Energy: {energy}%\n\n"
        "Every 30 minutes, 5% energy is added! ⏳\n"
        "For faster recovery, go to the shop and buy gems! 🛒"
    )
    
    await update.message.reply_text(text)

# 📌 تابع برای به‌روزرسانی انرژی
async def update_energy(context: ContextTypes.DEFAULT_TYPE):
    while True:
        try:
            for user_id, user_data in context.bot_data.get("user_data", {}).items():
                energy = user_data.get("energy", 100)
                if energy < 100:
                    user_data["energy"] = min(100, energy + 5)
                    save_data(context)
...
async def handle_payment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.photo or update.message.document:
        text = (
            "📸 فیش پرداخت دریافت شد! در حال بررسی توسط ادمین... ⏳\n"
            "بزودی جم‌ها به حسابت اضافه می‌شه! 💎"
        ) if language == "persian" else (
            "📸 Payment receipt received! Being reviewed by admin... ⏳\n"
            "Gems will be added to your account soon! 💎"
        )
        await update.message.reply_text(text)
        await context.bot.forward_message(ADMIN_ID, user_id, update.message.message_id)
    else:
        text = "⛔ لطفاً فیش پرداخت رو به صورت عکس یا فایل بفرست!" if language == "persian" else "⛔ Please send the payment receipt as a photo or file!"
        await update.message.reply_text(text)

# 📌 هندلر برای تأیید پرداخت توسط ادمین
async def confirm_payment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("لطفاً آیدی کاربر و تعداد جم رو وارد کنید! مثال: /confirm_payment 123456789 25")
        return
    
    try:
        target_id, gems = int(context.args[0]), int(context.args[1])
        user_data = context.bot_data["user_data"].get(target_id, {})
        language = user_data.get("language", "persian")
        
        user_data["gems"] = user_data.get("gems", 0) + gems
        user_data["pending_gems"] = 0
        save_data(context)
        
        text = f"💎 {gems} جم به حساب شما اضافه شد! 🎉" if language == "persian" else f"💎 {gems} Gems added to your account! 🎉"
        await context.bot.send_message(target_id, text)
        await update.message.reply_text(f"✅ {gems} جم برای کاربر {target_id} تأیید شد!")
    except (IndexError, ValueError):
        await update.message.reply_text("⛔ خطا: لطفاً آیدی کاربر و تعداد جم رو درست وارد کنید!")

# 📌 تنظیم هندلرها
def setup_handlers(application):
    application.add_handler(CommandHandler("start", start))
    application.add_handler(CallbackQueryHandler(handle_language_selection, pattern="^set_language_"))
    application.add_handler(MessageHandler(filters.Regex(r"^(⚔️ شروع بازی|⚔️ Start Game)$"), start_game))
    application.add_handler(MessageHandler(filters.Regex(r"^(🛒 فروشگاه|🛒 Shop)$"), shop))
    application.add_handler(MessageHandler(filters.Regex(r"^(🏴‍☠️ برترین ناخدایان|🏴‍☠️ Top Captains)$"), top_captains))
    application.add_handler(MessageHandler(filters.Regex(r"^(📕 اطلاعات کشتی|📕 Ship Info)$"), ship_info))
    application.add_handler(MessageHandler(filters.Regex(r"^(⚡️ انرژی جنگجویان|⚡️ Warriors' Energy)$"), warriors_energy))
    application.add_handler(MessageHandler(filters.Regex(r"^(💎 خرید جم|💎 Buy Gems)$"), buy_gems))
    application.add_handler(MessageHandler(filters.Regex(r"^(☄️ خرید توپ|☄️ Buy Cannons)$"), buy_cannons))
    application.add_handler(MessageHandler(filters.Regex(r"^(🛩️ خرید پهباد|🛩️ Buy Drones)$"), buy_drones))
    application.add_handler(MessageHandler(filters.Regex(r"^(دریانوردی ⛵️|Sailing ⛵️|توپ ☄️|Cannon ☄️|پهباد 🛩️|Drone 🛩️|استراتژی ⚔️|Strategy ⚔️|حمله گرایانه 🗡️|Aggressive 🗡️|دفاعی 🛡️|Defensive 🛡️|بازگشت به منو 🔙|Back to Menu 🔙)$"), handle_game_options))
    application.add_handler(MessageHandler(filters.Regex(r"^(0%|10%|20%|35%|50%|65%|80%|90%|100%)$"), handle_strategy_input))
    application.add_handler(CallbackQueryHandler(handle_cannon_purchase, pattern="^buy_.*_cannon"))
    application.add_handler(CallbackQueryHandler(handle_drone_purchase, pattern="^buy_.*_drone"))
    application.add_handler(CallbackQueryHandler(handle_friend_game, pattern="^(request_friend_game_|accept_friend_game_|reject_friend_game_|back_to_menu)"))
    application.add_handler(CallbackQueryHandler(buy_gems, pattern="^buy_.*_gems"))
    application.add_handler(CallbackQueryHandler(shop, pattern="^back_to_shop$"))
    application.add_handler(MessageHandler(filters.PHOTO | filters.Document.ALL, handle_payment))
...
async def main():
    ...
    application.job_queue.run_repeating(update_energy, interval=1800, first=10)
